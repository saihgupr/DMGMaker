...
class DMGEngine: ObservableObject {
    ...
    private func generateBackground(appName: String, size: NSSize, customBackgroundURL: URL?) -> NSImage {
        let view = DMGBackgroundView(appName: appName, appsIconPath: nil, customBackgroundURL: customBackgroundURL)
    ...
    func createDMG(appURL: URL, backgroundURL: URL?, outputName: String) {
        ...
        do {
            ...
            let bgImage = generateBackground(appName: outputName, size: NSSize(width: 600, height: 600), customBackgroundURL: backgroundURL)
